...
def _model():
    return os.environ.get("QORK_E2E_MODEL", os.environ.get("QORK_MODEL", "gpt-4.1-mini"))
...
def test_python_api_chat_plaintext(tmp_path, monkeypatch):
    ...
    text = ask("Say one short sentence.", stream=False, responses=False, plaintext=True, debug=False, return_text=True)
...
def test_python_api_responses_plaintext(tmp_path, monkeypatch):
    ...
    text1 = ask("Start a short thread via Responses API.", responses=True, plaintext=True, debug=True, return_text=True)
    assert isinstance(text1, str) and text1.strip(), "first responses call returned empty"

    text2 = ask("Continue the thread in one sentence.", responses=True, plaintext=True, debug=False, return_text=True)
