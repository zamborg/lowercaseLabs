...
final class StreamingWhisperSession: DictationSession {
    ...
    func append(_ buffer: AVAudioPCMBuffer, at _: AVAudioTime?) throws {
        ...
        lock.withLock {
            ...
            guard accumulatedSamples.count >= minimumPartialSampleCount else { return }
            guard partialTask == nil else { pendingPartial = true; return }
            let hasNewAudio = accumulatedSamples.count - lastRequestedPartialSampleCount >= partialStrideSampleCount
            guard latestText.isEmpty || hasNewAudio else { return }
            lastRequestedPartialSampleCount = accumulatedSamples.count
            snapshotToTranscribe = accumulatedSamples
        }
        if let snap = snapshotToTranscribe { startPartialTranscription(with: snap) }
    }

    func finish() async throws -> String {
        let taskToAwait = lock.withLock { () -> Task<Void, Never>? in
            isFinishing = true; pendingPartial = false; return partialTask
        }
        await taskToAwait?.value

        let snapshot = lock.withLock { accumulatedSamples }
        guard !snapshot.isEmpty else { throw DictationError.noSpeechDetected }

        let text = try await transcriber.transcribe(samples: snapshot, locale: locale)
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { throw DictationError.noSpeechDetected }

        lock.withLock { latestText = trimmed }
        await MainActor.run { [weak self] in self?.onUpdate?(DictationUpdate(text: trimmed, isFinal: true)) }
        return trimmed
    }

    func cancel() {
        let taskToCancel = lock.withLock { () -> Task<Void, Never>? in
            isCancelled = true; isFinishing = true; pendingPartial = false
            let t = partialTask; partialTask = nil
            accumulatedSamples.removeAll(keepingCapacity: false); latestText = ""
            return t
        }
        taskToCancel?.cancel()
    }
    ...
    private func startPartialTranscription(with snapshot: [Float]) {
        let task = Task { [weak self, transcriber, locale] in
            ...
            do { result = .success(try await transcriber.transcribe(samples: snapshot, locale: locale)) }
            catch is CancellationError { return }
            catch { result = .failure(error) }
            self?.handlePartialResult(result, snapshotSampleCount: snapshot.count)
        }
        lock.withLock { partialTask = task }
    }
    ...
    private func handlePartialResult(_ result: Result<String, Error>, snapshotSampleCount: Int) {
        ...
        lock.withLock {
            ...
            if case .success(let text) = result {
                ...
                if !t.isEmpty { latestText = t; update = DictationUpdate(text: t, isFinal: false) }
            }
            guard !isFinishing, !isCancelled else { pendingPartial = false; return }
            let hasPending = pendingPartial || accumulatedSamples.count - snapshotSampleCount >= partialStrideSampleCount
            pendingPartial = false
            guard hasPending, accumulatedSamples.count >= minimumPartialSampleCount else { return }
            lastRequestedPartialSampleCount = accumulatedSamples.count
            nextSnapshot = accumulatedSamples
        }

        if let update { DispatchQueue.main.async { [weak self] in self?.onUpdate?(update) } }
        if let nextSnapshot { startPartialTranscription(with: nextSnapshot) }
    }
}
