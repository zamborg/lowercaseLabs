...
final class PCMBufferSampleConverter {
    ...
    func samples(from buffer: AVAudioPCMBuffer) throws -> [Float] {
        if matchesOutputFormat(buffer.format) { return readFloatSamples(from: buffer) }
        ...
        guard let converter else {
            throw DictationError.audioCaptureFailed("Unable to prepare audio conversion.")
        }

        let ratio = outputFormat.sampleRate / buffer.format.sampleRate
        let expectedFrames = max(1, Int(ceil(Double(buffer.frameLength) * ratio)))
        guard let outputBuffer = AVAudioPCMBuffer(pcmFormat: outputFormat, frameCapacity: AVAudioFrameCount(expectedFrames)) else {
            throw DictationError.audioCaptureFailed("Unable to allocate conversion buffer.")
        }

        var didProvideInput = false
        var conversionError: NSError?
        let status = converter.convert(to: outputBuffer, error: &conversionError) { _, outputStatus in
            if didProvideInput { outputStatus.pointee = .noDataNow; return nil }
            didProvideInput = true; outputStatus.pointee = .haveData; return buffer
        }

        if let err = conversionError { throw DictationError.audioCaptureFailed(err.localizedDescription) }
        guard status != .error else { throw DictationError.audioCaptureFailed("Audio conversion failed.") }
